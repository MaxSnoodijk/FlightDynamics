...
def main(path, interval=60, fuselage=True):
    solver = AircraftStability(path=path, fuselage=fuselage)

    solver.plot_eigenvalues(show=False)
    solver.plot_responses(deflections=[0.1, 0], deflections_time=1, stop=interval, show=True)

    solver.plot_eigenvalues(symmetric=False, show=False)
    solver.plot_responses(deflections=[0.1, 0], deflections_time=1, stop=interval / 4, symmetric=False, show=True)
